/*
 * @Author: your name
 * @Date: 2021-03-16 16:04:20
 * @LastEditTime: 2021-03-16 19:04:30
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /Demo/extension/mish/src/vision.cpp
 */
...
torch::Tensor mish_forward(const torch::Tensor &input,
                           const at::optional<torch::Tensor> out) {
...
torch::Tensor mish_backward(const torch::Tensor &input,
                            const torch::Tensor &grad_out) {
  auto input_arg = torch::TensorArg(input, "input", 0);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("mish_forward", &mish_forward, "Mish activation forward", "input"_a,
        "out"_a = nullptr);
  m.def("mish_backward", &mish_backward, "Mish activation backward", "input"_a,
        "grad_out"_a);
}