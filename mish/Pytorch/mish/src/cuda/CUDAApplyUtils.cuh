...
// Taken from
// https://github.com/pytorch/pytorch/blob/ff6cda0da6d8fceadbe0cf31ef73c78d3c9e9bcc/aten/src/ATen/cuda/CUDAApplyUtils.cuh
...
template <typename T1, typename IndexType, typename T2 = void,
...
template <typename Op, typename scalar, typename IndexType, int ADims,
          int remaining_steps, typename... Offsets>
struct ApplyOp1 {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar, IndexType> &a, const Op &op, int n,
      IndexType linearIndex, Offsets... aOffsets) {
    // Convert `linearIndex` into an offset of `a`
    const IndexType aOffset =
        sizeof...(Offsets) < n
            ? detail::IndexToOffset<scalar, IndexType, ADims>::get(linearIndex,
                                                                   a)
            : 0;

    ApplyOp1<Op, scalar, IndexType, ADims, remaining_steps - 1, const IndexType,
             Offsets...>::apply(a, op, n, linearIndex + 1, aOffsets...,
                                aOffset);
  }
};
...
struct ApplyOp1<Op, scalar, IndexType, ADims, 0, Offset> {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar, IndexType> &a, const Op &op, int n,
      IndexType linearIndex, Offset offset) {
    op(a.data[offset]);
  }
};
...
__global__ void kernelPointwiseApply1(detail::TensorInfo<scalar, IndexType> a,
  ...
  for (IndexType linearIndex = (blockIdx.x * blockDim.x + threadIdx.x) * step;
    ...
    ApplyOp1<Op, scalar, IndexType, ADims, step>::apply(
        a, op, ::min(step, static_cast<int>(totalElements - linearIndex)),
        linearIndex);
  }
}

template <typename Op, typename scalar1, typename scalar2, typename IndexType,
          int ADims, int BDims, int remaining_steps, typename... Offsets>
struct ApplyOp2 {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b, const Op &op, int n,
      IndexType linearIndex, Offsets... aOffsets, Offsets... bOffsets) {
    // Convert `linearIndex` into an offset of `a`
    const IndexType aOffset =
        sizeof...(Offsets) < n
            ? detail::IndexToOffset<scalar1, IndexType, ADims>::get(linearIndex,
                                                                    a)
            : 0;
    ...
    const IndexType bOffset =
        sizeof...(Offsets) < n
            ? detail::IndexToOffset<scalar2, IndexType, BDims>::get(linearIndex,
                                                                    b)
            : 0;

    ApplyOp2<Op, scalar1, scalar2, IndexType, ADims, BDims, remaining_steps - 1,
             const IndexType, Offsets...>::apply(a, b, op, n, linearIndex + 1,
                                                 aOffsets..., aOffset,
                                                 bOffsets..., bOffset);
  }
};
...
// We don't need to pass in how many elements need to processed in this case.
template <typename Op, typename scalar1, typename scalar2, typename IndexType,
          int ADims, int BDims, typename Offset>
struct ApplyOp2<Op, scalar1, scalar2, IndexType, ADims, BDims, 0, Offset> {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b, const Op &op, int n,
      IndexType linearIndex, Offset aOffset, Offset bOffset) {
    op(a.data[aOffset], b.data[bOffset]);
  }
};

template <typename Op, typename scalar1, typename scalar2, typename IndexType,
          int ADims, int BDims, typename... Offsets>
...
template <typename Op, typename scalar1, typename scalar2, typename IndexType,
          int ADims, int BDims, int step>
#if __CUDA_ARCH__ >= 350 || defined __HIP_PLATFORM_HCC__
C10_LAUNCH_BOUNDS_2(AT_APPLY_THREADS_PER_BLOCK, AT_APPLY_BLOCKS_PER_SM)
#endif
__global__ void kernelPointwiseApply2(detail::TensorInfo<scalar1, IndexType> a,
...
template <typename Op, typename scalar1, typename scalar2, typename scalar3,
          typename IndexType, int ADims, int BDims, int CDims,
          int remaining_steps, typename... Offsets>
struct ApplyOp3 {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b,
      detail::TensorInfo<scalar3, IndexType> &c, const Op &op, int n,
      IndexType linearIndex, Offsets... aOffsets, Offsets... bOffsets,
      Offsets... cOffsets) {
    // Convert `linearIndex` into an offset of `a`
    const IndexType aOffset =
    ...
    const IndexType cOffset =
        sizeof...(Offsets) < n
            ? detail::IndexToOffset<scalar3, IndexType, CDims>::get(linearIndex,
                                                                    c)
            : 0;

    ApplyOp3<Op, scalar1, scalar2, scalar3, IndexType, ADims, BDims, CDims,
             remaining_steps - 1, const IndexType,
             Offsets...>::apply(a, b, c, op, n, linearIndex + 1, aOffsets...,
                                aOffset, bOffsets..., bOffset, cOffsets...,
                                cOffset);
  }
};
...
// We don't need to pass in how many elements need to processed in this case.
template <typename Op, typename scalar1, typename scalar2, typename scalar3,
          typename IndexType, int ADims, int BDims, int CDims, typename Offset>
struct ApplyOp3<Op, scalar1, scalar2, scalar3, IndexType, ADims, BDims, CDims,
                0, Offset> {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b,
      detail::TensorInfo<scalar3, IndexType> &c, const Op &op, int n,
      IndexType linearIndex, Offset aOffset, Offset bOffset, Offset cOffset) {
...
template <typename Op, typename scalar1, typename scalar2, typename scalar3,
          typename IndexType, int ADims, int BDims, int CDims, int step>
#if __CUDA_ARCH__ >= 350 || defined __HIP_PLATFORM_HCC__
C10_LAUNCH_BOUNDS_2(AT_APPLY_THREADS_PER_BLOCK, AT_APPLY_BLOCKS_PER_SM)
#endif
__global__ void kernelPointwiseApply3(detail::TensorInfo<scalar1, IndexType> a,
                                      detail::TensorInfo<scalar2, IndexType> b,
  ...
  for (IndexType linearIndex = (blockIdx.x * blockDim.x + threadIdx.x) * step;
    ...
    ApplyOp3<Op, scalar1, scalar2, scalar3, IndexType, ADims, BDims, CDims,
             step>::apply(a, b, c, op,
                          ::min(step,
                                static_cast<int>(totalElements - linearIndex)),
                          linearIndex);
  }
}

template <typename Op, typename scalar1, typename scalar2, typename scalar3,
          typename scalar4, typename IndexType, int ADims, int BDims, int CDims,
          int DDims, int remaining_steps, typename... Offsets>
struct ApplyOp4 {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b,
      detail::TensorInfo<scalar3, IndexType> &c,
      detail::TensorInfo<scalar4, IndexType> &d, const Op &op, int n,
      IndexType linearIndex, Offsets... aOffsets, Offsets... bOffsets,
    ...
    const IndexType dOffset =
        sizeof...(Offsets) < n
            ? detail::IndexToOffset<scalar4, IndexType, DDims>::get(linearIndex,
                                                                    d)
            : 0;

    ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType, ADims, BDims,
             CDims, DDims, remaining_steps - 1, const IndexType,
             Offsets...>::apply(a, b, c, d, op, n, linearIndex + 1, aOffsets...,
                                aOffset, bOffsets..., bOffset, cOffsets...,
                                cOffset, dOffsets..., dOffset);
  }
};
...
template <typename Op, typename scalar1, typename scalar2, typename scalar3,
          typename scalar4, typename IndexType, int ADims, int BDims, int CDims,
          int DDims, typename Offset>
struct ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType, ADims, BDims,
                CDims, DDims, 0, Offset> {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b,
      detail::TensorInfo<scalar3, IndexType> &c,
...
template <typename Op, typename scalar1, typename scalar2, typename scalar3,
          typename scalar4, typename IndexType, int ADims, int BDims, int CDims,
          int DDims, typename... Offsets>
struct ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType, ADims, BDims,
                CDims, DDims, 0, Offsets...> {
  __device__ __forceinline__ static void apply(
      detail::TensorInfo<scalar1, IndexType> &a,
      detail::TensorInfo<scalar2, IndexType> &b,
      detail::TensorInfo<scalar3, IndexType> &c,
...
C10_LAUNCH_BOUNDS_2(AT_APPLY_THREADS_PER_BLOCK, AT_APPLY_BLOCKS_PER_SM)
#endif
__global__ void kernelPointwiseApply4(detail::TensorInfo<scalar1, IndexType> a,
                                      detail::TensorInfo<scalar2, IndexType> b,
                                      detail::TensorInfo<scalar3, IndexType> c,
  ...
  for (IndexType linearIndex = (blockIdx.x * blockDim.x + threadIdx.x) * step;
       ...
       linearIndex += gridDim.x * blockDim.x * step) {
    ApplyOp4<Op, scalar1, scalar2, scalar3, scalar4, IndexType, ADims, BDims,
             CDims, DDims, step>::apply(a, b, c, d, op,
                                        ::min(step,
...
inline bool getApplyGrid(uint64_t totalElements, dim3 &grid,
  ...
  if (numBlocks > maxGridX) numBlocks = maxGridX;
  grid = dim3(numBlocks);
  return true;
}

inline dim3 getApplyBlock() { return dim3(AT_APPLY_THREADS_PER_BLOCK); }

template <typename scalar, int step, typename Op>
inline bool CUDA_tensor_apply1(at::Tensor a, const Op op,
...
#define HANDLE_CASE(TYPE, A)                                           \
  kernelPointwiseApply1<Op, scalar, TYPE, A, step>                     \
...
inline bool CUDA_tensor_apply2(at::Tensor a, at::Tensor b, const Op op,
                               TensorArgType aType = TensorArgType::ReadWrite,
...
#define HANDLE_CASE(TYPE, A, B)                                        \
  kernelPointwiseApply2<Op, scalar1, scalar2, TYPE, A, B, step>        \
      <<<grid, block, 0, at::cuda::getCurrentCUDAStream(curDevice)>>>( \
...
template <typename scalar1, typename scalar2, typename scalar3, int step,
          typename Op>
inline bool CUDA_tensor_apply3(at::Tensor a, at::Tensor b, at::Tensor c,
                               const Op op,
                               TensorArgType aType = TensorArgType::ReadWrite,
                               TensorArgType bType = TensorArgType::ReadOnly,
  ...
  if (totalElements != b.numel() || totalElements != c.numel()) {
    return false;
  }
...
template <typename scalar1, typename scalar2, typename scalar3,
          typename scalar4, int step, typename Op>
inline bool CUDA_tensor_apply4(at::Tensor a, at::Tensor b, at::Tensor c,
                               at::Tensor d, const Op op,
                               TensorArgType aType = TensorArgType::ReadWrite,
                               TensorArgType bType = TensorArgType::ReadOnly,
                               TensorArgType cType = TensorArgType::ReadOnly,
  ...
  if (a.dim() > MAX_TENSORINFO_DIMS || b.dim() > MAX_TENSORINFO_DIMS ||
      c.dim() > MAX_TENSORINFO_DIMS || d.dim() > MAX_TENSORINFO_DIMS) {
    return false;
  }
...
#define HANDLE_CASE(TYPE, A, B, C, D)                                          \
  kernelPointwiseApply4<Op, scalar1, scalar2, scalar3, scalar4, TYPE, A, B, C, \
                        D, step>                                               \
      <<<grid, block, 0, at::cuda::getCurrentCUDAStream(curDevice)>>>(         \
...
#define HANDLE_A_CASE(TYPE, A, B, C, D)   \
  ...
  if (detail::canUse32BitIndexMath(a) && detail::canUse32BitIndexMath(b) &&
      detail::canUse32BitIndexMath(c) && detail::canUse32BitIndexMath(d)) {
    detail::TensorInfo<scalar1, unsigned int> aInfo =
        detail::getTensorInfo<scalar1, unsigned int>(a);
