/*
 * @Author: your name
 * @Date: 2021-03-16 16:43:20
 * @LastEditTime: 2021-03-16 16:55:08
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /Demo/extension/mish/src/cuda/mish_kernel.cu
 */
...
void mish_backward(torch::Tensor &grad_inp, const torch::Tensor &input,
  ...
  CUDA_tensor_apply3<scalar_t, scalar_t, scalar_t>(
      ...
      [=] __host__ __device__(scalar_t & grad_inp, const scalar_t &inp,
                              const scalar_t &grad_out) {
        mish_bwd_func(grad_inp, inp, grad_out);
      },
      TensorArgType::ReadWrite, TensorArgType::ReadOnly,
      TensorArgType::ReadOnly);
}
...
void mish_forward_cuda(torch::Tensor &output, const torch::Tensor &input) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "mish_forward_cuda",
      [&] { kernel::mish_forward<scalar_t>(output, input); });
}

void mish_backward_cuda(torch::Tensor &grad_inp, const torch::Tensor &input,
                        const torch::Tensor &grad_out) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_inp.scalar_type(), "mish_backward_cuda",
